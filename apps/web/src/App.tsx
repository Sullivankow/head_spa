...
export default function App() {
  ...
  return <div className="min-h-screen overflow-x-hidden">
    <header className="fixed top-0 z-50 w-full border-b border-black/5 bg-cream/90 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8">
        <a href="#" className="font-display text-3xl font-semibold tracking-tight">Maison Sereine</a>
        <nav className="hidden items-center gap-8 text-sm md:flex">
          <a href="#experience" className="hover:opacity-60">L’expérience</a>
          <a href="#soins" className="hover:opacity-60">Les rituels</a>
          <a href="#faq" className="hover:opacity-60">FAQ</a>
          <a href="#reservation" className="rounded-full bg-ink px-5 py-3 text-white transition hover:-translate-y-0.5">Réserver <ArrowUpRight className="ml-1 inline h-4 w-4" /></a>
        </nav>
        <button aria-label="Ouvrir le menu" className="md:hidden" onClick={() => setMenu(!menu)}>{menu ? <X /> : <Menu />}</button>
      </div>
      {menu && <nav className="flex flex-col gap-5 border-t border-black/5 px-5 py-5 md:hidden">
        <a href="#experience" onClick={() => setMenu(false)}>L’expérience</a><a href="#soins" onClick={() => setMenu(false)}>Les rituels</a><a href="#faq" onClick={() => setMenu(false)}>FAQ</a><a href="#reservation" onClick={() => setMenu(false)} className="font-semibold">Réserver →</a>
      </nav>}
    </header>

    <main>
      <section className="relative flex min-h-[92vh] items-end overflow-hidden pt-24">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_75%_25%,#e8ddd0_0,transparent_34%),linear-gradient(120deg,#f7f2eb_0%,#efe6dc_55%,#e4d7ca_100%)]" />
        <div className="relative mx-auto grid w-full max-w-7xl gap-12 px-5 pb-16 lg:grid-cols-[1.05fr_.95fr] lg:px-8 lg:pb-24">
          <div className="self-end">
            <div className="mb-7 inline-flex items-center gap-2 rounded-full border border-black/10 bg-white/40 px-4 py-2 text-xs tracking-[.18em]"><Sparkles className="h-4 w-4" /> HEADSPA · BIEN-ÊTRE</div>
            <h1 className="max-w-4xl font-display text-6xl leading-[.88] tracking-tight sm:text-7xl lg:text-8xl">Ralentir.<br /><i>Respirer.</i><br />Se retrouver.</h1>
            ...
            <div className="mt-9 flex flex-wrap gap-3"><a href="#reservation" className="rounded-full bg-ink px-7 py-4 text-sm font-semibold text-white transition hover:-translate-y-1">Je réserve mon rituel</a><a href="#soins" className="rounded-full border border-black/15 bg-white/30 px-7 py-4 text-sm">Découvrir les soins</a></div>
          ...
          <div className="relative min-h-[360px] overflow-hidden rounded-[2.5rem] bg-[#cbb6a8] shadow-2xl lg:min-h-[570px]">
            ...
            <div className="absolute bottom-8 left-8 right-8 rounded-3xl border border-white/40 bg-white/25 p-6 backdrop-blur-md">
      ...
      <section className="border-y border-black/5 bg-white/45 py-5"><div className="mx-auto flex max-w-7xl flex-wrap justify-center gap-x-10 gap-y-3 px-5 text-xs font-semibold uppercase tracking-[.18em] text-black/50"><span>Massage du cuir chevelu</span><span>•</span><span>Vapeur douce</span><span>•</span><span>Relaxation profonde</span><span>•</span><span>Beauté naturelle</span></div></section>

      <section id="experience" className="mx-auto max-w-7xl px-5 py-24 lg:px-8 lg:py-32">
        <div className="grid gap-16 lg:grid-cols-[.8fr_1.2fr]">
          <div><p className="mb-4 text-xs font-semibold uppercase tracking-[.2em] text-sage">L’expérience Maison Sereine</p><h2 className="font-display text-5xl leading-none sm:text-6xl">Le luxe de<br /><i>ne rien faire.</i></h2></div>
      ...
      <section className="mx-auto max-w-7xl px-5 py-24 lg:px-8 lg:py-32">
        <div className="rounded-[2.5rem] bg-ink p-8 text-white sm:p-12 lg:p-16">
          <div className="grid gap-12 lg:grid-cols-[1fr_.7fr]">
            <div><Heart className="mb-10 h-7 w-7" /><h2 className="max-w-2xl font-display text-5xl leading-none sm:text-6xl">« J’ai enfin pris<br /><i>le temps.</i> »</h2><p className="mt-7 max-w-lg text-sm leading-7 text-white/60">Le headspa n’est pas seulement un soin. C’est un rendez-vous avec soi-même, dans une cabine pensée pour ralentir.</p></div>
            <div className="flex items-end"><div className="border-l border-white/20 pl-6"><p className="font-display text-3xl">Une heure où le monde peut attendre.</p><p className="mt-3 text-xs uppercase tracking-[.18em] text-white/50">Maison Sereine</p></div></div>
          </div>
        </div>
      </section>
    ...
    <footer className="border-t border-black/5 px-5 py-10"><div className="mx-auto flex max-w-7xl flex-col justify-between gap-5 text-xs text-black/50 sm:flex-row"><span className="font-display text-2xl text-ink">Maison Sereine</span><span>© 2026 · Mentions légales · Politique de confidentialité</span></div></footer>
